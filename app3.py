...
def load_google_creds():
    # 1) Native Streamlit secrets (works with .streamlit/secrets.toml or Streamlit Cloud)
    if "GOOGLE_CREDENTIALS" in st.secrets:
        return st.secrets["GOOGLE_CREDENTIALS"]

    # 2) Render Secret File named 'secrets.toml' at project root
    if os.path.exists("secrets.toml"):
        data = toml.load("secrets.toml")
        if "GOOGLE_CREDENTIALS" in data:
            return data["GOOGLE_CREDENTIALS"]

    # 3) Environment variable with one-line JSON
    env_json = os.getenv("GOOGLE_CREDENTIALS")
    if env_json:
        return json.loads(env_json)

    raise RuntimeError(
        "Google credentials not found. Provide via st.secrets, a root 'secrets.toml', "
        "or an env var GOOGLE_CREDENTIALS."
    )
...
def connect_gsheets():
    ...
    # use .worksheet("SheetName") if you target a specific tab
